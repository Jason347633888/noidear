  ...
  <div class="recall-detail-page" v-loading="loading">
    <div class="page-header" v-if="recall">
      <div class="header-left">
        ...
        <el-tag :type="statusTagType(recall.status)" size="small">{{ STATUS_MAP[recall.status] ?? recall.status }}</el-tag>
        <el-tag :type="riskTagType(recall.risk_level)" size="small" class="ml-8">{{ RISK_MAP[recall.risk_level] ?? recall.risk_level }}</el-tag>
      </div>
    ...
    <template v-if="recall">
      <el-card class="section-card">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="风险等级">{{ RISK_MAP[recall.risk_level] ?? recall.risk_level }}</el-descriptions-item>
          ...
          <el-descriptions-item label="追溯快照引用" :span="2">
            <span v-if="recall.source_query_ref" class="snapshot-ref">
              {{ recall.source_query_ref }}
              <el-tag type="success" size="small" style="margin-left: 8px">范围已锁定</el-tag>
            </span>
            <span v-else>未关联快照</span>
      ...
      <el-card v-if="recall.evidence?.length" class="section-card">
