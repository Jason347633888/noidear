...
business_chain:
  ...
  - SupplierEvaluation → Supplier.evaluationStatus
...
4. **供应商评估**：`SupplierEvaluation` 定期评分，联动更新 `Supplier.evaluationStatus`。
...
| `SupplierEvaluation` | `server/src/modules/supplier-evaluation/supplier-evaluation.service.ts` | 评分后联动写 `Supplier.evaluationStatus` `已验证` |
...
|---|---|---|---|---|
| 1 | 采购员确认供应商已在台账中 | `Supplier` 存在，含有效 `evaluationStatus: approved` | SupplierService | 无供应商主数据则入库单无法关联溯源 |
...
| 5 | 仓管员或系统查询批次状态 | `MaterialBatch.lotStatus` 反映库存状态 | BatchService | — |
| 6 | 品质管理员按周期提交 `SupplierEvaluation` | `Supplier.evaluationStatus` 更新为 `approved/suspended/eliminated` | SupplierEvaluationService | 无评估则供应商准入状态不更新，无法支持再评价管理 |
...
Supplier（主数据）
  ...
  └── SupplierEvaluation（supplier_id FK → 联动更新 Supplier.evaluationStatus）
...
| GAP-103 | `Supplier` 有两个状态字段：`status`（`active/disabled`，运营激活状态）和 `evaluationStatus`（`approved/suspended/eliminated/pending`，DB: `supplier_status`，食品安全资质评估状态）；两个字段独立，语义不重叠 | 已通过 PR3 enum 化并改名 `evaluationStatus`，DB 列名 `@map("supplier_status")` 保留 | `evaluationStatus: eliminated` 时下游入库/领料门禁检查 `evaluationStatus !== 'eliminated'` | P1 | 已验证 | `server/src/prisma/schema.prisma` Supplier model；`supplier-access.service.ts` |
...
| GAP-103 | `status`（运营）与 `evaluationStatus`（资质评估，DB: `supplier_status`）职责已明确；在入库、领料前加门禁检查 `evaluationStatus !== 'eliminated'` | warehouse/InboundService, RequisitionService | 否 | `fix/supplier-status-gate` | 可 |
...
- `server/src/modules/supplier-evaluation/supplier-evaluation.service.ts` — 评估 + 联动 evaluationStatus
...
|---|---|---|---|---|
| 供应商主数据 | `Supplier`（`suppliers` 表） | name, supplierCode, evaluationStatus | 不允许在 MaterialBatch、IncomingInspection 等下游表重新维护供应商名称 | `ExternalParty` 不得承接供应商功能 |
| 物料批次 | `MaterialBatch`（`material_batches` 表）| batchNumber, lotStatus, quantity, expiryDate | 不允许在 IncomingInspection 或 BatchMaterialUsage 中维护批次数量副本 | — |
...
| P1 | GAP-103 | `fix/supplier-status-gate` | 无 | 是 | InboundService、RequisitionService 在操作前验证 `evaluationStatus !== 'eliminated'` |
