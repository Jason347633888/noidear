...
| GAP-410 | 内审不符合项（AuditFinding）整改完成后，当前系统无自动触发 `CorrectiveAction`（CAPA）的逻辑。业务上不符合项应进入 CAPA 闭环，但 `AuditFinding` 与 `CorrectiveAction` 之间无关联字段 | 内审模块实现时未与 CAPA 模块设计整合 | 内审不符合项整改完成只在内审内部闭环（verified 状态），不进入全厂 CAPA 系统追踪，管理评审无法汇总"内审触发的 CAPA 数量" | P1 | 已验证 | `server/src/prisma/schema.prisma` 第 2192-2236 行 `AuditFinding` 模型无 `correctiveActionId` 字段；`server/src/prisma/schema.prisma` 第 3055 行 CorrectiveAction.trigger_type 有 `internal_audit` 枚举值但无外键关联 |
...
| GAP-410 | 在 AuditFinding 模型增加 `correctiveActionId String?` 可选字段；在整改验证通过（verified）时，若 issueType 为重大不符合，自动创建 CorrectiveAction 并回填 correctiveActionId | CorrectiveAction 模块 | 否 | feat/audit-finding-to-capa-linkage | 否（需先确认 CAPA 触发规则） |
...
| P1 | GAP-410 | feat/audit-finding-to-capa-linkage | CorrectiveAction 模块 trigger_type 枚举已有 internal_audit | 否 | 验证内审不符合项 verified 后，CorrectiveAction 表中出现对应记录，`trigger_type = 'internal_audit'`，`source_id = AuditFinding.id` |
