...
| GAP-203 | P1 | 已验证 | 是 | 是 | 是 | `executing-plans` | docs/superpowers/specs/2026-05-01-gap-203-packaging-material-usage-batch-fk-design.md | docs/superpowers/plans/2026-05-01-gap-203-packaging-material-usage-batch-fk-implementation.md | spec 和 plan 已写完；执行时只使用 executing-plans |
| GAP-204 | P2 | 已验证 | 是 | 是 | 是 | `executing-plans` | docs/superpowers/specs/2026-05-02-gap-204-mixing-execution-shift-type-fk-design.md | docs/superpowers/plans/2026-05-02-gap-204-mixing-execution-shift-type-fk-implementation.md | spec 和 plan 已写完；执行时只使用 executing-plans，MixingExecution.shift_type_id 补齐到 ShiftType 的可选 FK |
...
| GAP-305 | P1 | 已验证 | 是 | 是 | 是 | `executing-plans` | docs/superpowers/specs/2026-05-01-gap-305-ccp-deviation-nonconformance-design.md | docs/superpowers/plans/2026-05-01-gap-305-ccp-deviation-nonconformance-implementation.md | spec 和 plan 已写完；执行时只使用 executing-plans，CCP 偏差自动触发 NonConformance |
...
| GAP-308 | P1 | 已验证 | 是 | 是 | 是 | `executing-plans` | docs/superpowers/specs/2026-05-01-gap-308-traceability-snapshot-persistence-design.md | docs/superpowers/plans/2026-05-01-gap-308-traceability-snapshot-persistence-implementation.md | spec 和 plan 已写完；执行时只使用 executing-plans，聚焦 TraceabilitySnapshot 持久化，不实现 GAP-307 完整查询链 |
| GAP-309 | P1 | 已验证 | 是 | 是 | 是 | `executing-plans` | docs/superpowers/specs/2026-05-02-gap-309-customer-complaint-customer-fk-design.md | docs/superpowers/plans/2026-05-02-gap-309-customer-complaint-customer-fk-implementation.md | spec 和 plan 已修正；执行时只使用 executing-plans，CustomerComplaint 必须引用 ExternalParty customer 主数据，并同步修复 ExternalParty API JWT companyId 租户隔离 |
...
| GAP-313 | P1 | 已验证 | 是 | 是 | 是 | `brainstorming` -> `grill-with-docs` -> `writing-plans` -> `executing-plans` | docs/superpowers/specs/2026-05-02-gap-313-nc-source-validation-and-index-design.md | docs/superpowers/plans/2026-05-02-gap-313-nc-source-validation-and-index-implementation.md | spec 和 plan 已写完；执行时只使用 executing-plans，应用层校验 NC source_type/source_id；production_batch 通过 ProductionBatch.productId -> Product.company_id 校验租户归属，覆盖手工 create 与 createFromCcpDeviation 自动路径，并保留 tenant-scoped source lookup index |
| GAP-314 | P1 | 已验证 | 是 | 是 | 是 | `brainstorming` -> `grill-with-docs` -> `writing-plans` -> `executing-plans` | docs/superpowers/specs/2026-05-02-gap-314-safe-sequence-number-generation-design.md | docs/superpowers/plans/2026-05-02-gap-314-safe-sequence-number-generation-implementation.md | spec 和 plan 已修正；执行时只使用 executing-plans，新增数据库序列表替代手工 NC、CCP 偏差自动 NC、来料检验失败自动 NC、CAPA 的 count()+1 编号 |
| GAP-315 | P1 | 已验证 | 是 | 是 | 是 | `brainstorming` -> `grill-with-docs` -> `writing-plans` -> `executing-plans` | docs/superpowers/specs/2026-05-02-gap-315-nc-dispose-to-rework-record-design.md | docs/superpowers/plans/2026-05-02-gap-315-nc-dispose-to-rework-record-implementation.md | spec 和 plan 已写完；执行时只使用 executing-plans，生产批次来源 NC 处置为 rework 时事务内自动创建 pending ReworkRecord |
