...
| GAP-314 | P1 | 已验证 | 是 | 是 | 是 | `brainstorming` -> `grill-with-docs` -> `writing-plans` -> `executing-plans` | docs/superpowers/specs/2026-05-02-gap-314-safe-sequence-number-generation-design.md | docs/superpowers/plans/2026-05-02-gap-314-safe-sequence-number-generation-implementation.md | spec 和 plan 已写完；执行时只使用 executing-plans，新增数据库序列表替代 NC/CAPA 的 count()+1 编号 |
