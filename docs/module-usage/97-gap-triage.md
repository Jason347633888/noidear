...
| GAP-313 | P1 | 已验证 | 是 | 是 | 是 | `brainstorming` -> `grill-with-docs` -> `writing-plans` -> `executing-plans` | docs/superpowers/specs/2026-05-02-gap-313-nc-source-validation-and-index-design.md | docs/superpowers/plans/2026-05-02-gap-313-nc-source-validation-and-index-implementation.md | spec 和 plan 已写完；执行时只使用 executing-plans，应用层校验 NC source_type/source_id 并保留 tenant-scoped source lookup index |
