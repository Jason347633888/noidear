...
approvals (
  ...
  -- 支持会签和依次审批
  approval_type   VARCHAR(20)   DEFAULT 'single',  -- single/countersign/sequential
  sequence        INT           DEFAULT 1,         -- 审批顺序（依次审批时使用）
  group_id        VARCHAR(50),                     -- 会签组 ID（会签时使用）
...
- [ ] approvals 表包含所有字段：
  - id, document_id, record_id, approver_id, status, comment, created_at
  - approval_type, sequence, group_id（支持会签和依次审批）
...
- [ ] 索引配置正确（document_id, record_id, approver_id, status, group_id）
- [ ] 数据库迁移文件生成

**技术要点**:
- status 字段使用枚举类型（approved, rejected）
- document_id 和 record_id 二选一，用于关联不同类型的资源
- approval_type 支持三种模式：single（单级）、countersign（会签）、sequential（依次审批）
- sequence 用于依次审批的顺序控制
- group_id 用于会签的分组
...
**依赖**: TASK-053（需要先扩展 approvals 表）, TASK-055, TASK-056
