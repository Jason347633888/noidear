...
- Modify `tools/generate-system-map.py`: recursive API scan, direct request scan, route parser fixes, deleted-scope categories.
...
**Files:**
- Modify: `tools/generate-system-map.py`
...
- [ ] **Step 2: Replace API file glob**

In `load_all_frontend_apis()`, replace the `CLIENT_API_DIR.glob("*.ts")` loop with recursive scanning:
...
for f in sorted(CLIENT_API_DIR.rglob("*.ts")):
    ...
    calls = parse_frontend_api_file(f)
    if calls:
        modules[name] = calls
```

- [ ] **Step 3: Add direct client request scan**
...
- [ ] **Step 4: Fix backend controller parsing without cross-file backtracking**

Replace the controller prefix regex with support for `@Controller()`, then parse route decorators by iterating route matches and inspecting only the text slice until the next route decorator. Do not use a `DOTALL` negative-lookahead pattern over the whole file.
...
- [ ] **Step 5: Ignore TypeScript indexed-access strings**

Inside `parse_frontend_api_file`, skip URL candidates that do not start with `/` or a template expression that begins with `/`:
...
- [ ] **Step 6: Add deleted-scope classification**
...
- [ ] **Step 7: Wire classification into the main module builder**

Change `build_module_data(...)` so every frontend call receives a `status`. This is the point where `classify_call(...)` must be called.
...
Update `main()` to load `deleted = load_deleted_scopes()` and `direct_requests = load_direct_client_requests()`, then pass both into `build_module_data(...)`.

- [ ] **Step 8: Emit counters for every status**

Add status counters to stdout and the HTML data payload:

```python
def count_statuses(modules: list, deleted: dict) -> dict:
...
Call it as `count_statuses(modules, deleted)` after `modules` is built.
