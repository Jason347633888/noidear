...
- **grill-with-docs:** 已对照 `docs/MASTER_DATA_AND_TRACEABILITY_MODEL.md` 与 `docs/module-usage/09-nonconformance-capa.md` 校准。新增表只是序列游标，不是新的不合格、CAPA、批次或追溯事实源；不触碰 `ProductionBatch / MaterialBatch / BatchMaterialUsage / InventoryMovement` 主链；历史数据不重排；已按 review blocker 覆盖 `createFromCcpDeviation()` 和 `WorkflowTriggersService.handleInspectionFail()`；该 PR 可独立执行。
