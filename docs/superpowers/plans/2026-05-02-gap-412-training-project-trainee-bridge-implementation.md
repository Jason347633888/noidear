...
- **grill-with-docs 校准结论：** 本 GAP 复用当前代码中的 `User` 作为业务 Employee 口径落点，不新增人员主数据；培训模块不进入 `ProductionBatch / MaterialBatch / BatchMaterialUsage / InventoryMovement` 追溯主链。
...
- **历史数据停止条件：** 如果 migration preflight 发现 `training_projects.trainees` 中存在 orphan `User.id` 或空学员数组，不得自动修复；停止并回报需要业务确认。
...
In the `mockPrisma` object in `beforeEach`, add `trainingProjectTrainee`, `user`, and `learningRecord.deleteMany`.
...
In `createProject()`, remove `trainees: dto.trainees` from `trainingProject.create`, validate trainees before create, write bridge rows after create, and return the hydrated project:
...
```ts
    ...
    await this.prisma.trainingProjectTrainee.createMany({
    ...
    await this.createLearningRecordsForTrainees(project.id, traineeIds);
    await this.createTodoTasksForProject(project, traineeIds);
...
```ts
    ...
    if (trainees) {
      const traineeIds = this.normalizeTraineeIds(trainees);
      await this.validateActiveTrainees(traineeIds);
      await this.syncTraineeLinks(id, traineeIds);
      await this.syncLearningRecords(id, traineeIds);
    }
...
```ts
  private async syncTraineeLinks(projectId: string, traineeIds: string[]) {
    await this.prisma.trainingProjectTrainee.deleteMany({
      where: {
        projectId,
...
```ts
  async addTrainees(id: string, userIds: string[]) {
    ...
    if (toCreate.length > 0) {
      ...
      await this.createLearningRecordsForTrainees(id, toCreate);
      await this.createTodoTasksForNewTrainees(project, toCreate);
    }
...
Keep the existing `learningRecord.deleteMany()` and `todoTask.deleteMany()` cleanup, and replace `return updated;` with:
...
- [ ] **Step 11: Update todo helper signatures**
...
```ts
  private async createTodoTasksForProject(project: any, traineeIds: string[]) {
```
...
Keep `createTodoTasksForNewTrainees(project, userIds)` unchanged.
...
- [ ] **Step 2: Add add trainee bridge assertion**
...
- [ ] **Step 3: Add remove trainee bridge assertion**
