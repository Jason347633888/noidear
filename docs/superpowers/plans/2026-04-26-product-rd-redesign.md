...
**Architecture:** 后端新增 `ProcessStepApproval` 表支持多人会签，审批通过后由后端触发步骤推进和副作用（创建Product、写Recipe）；前端完全重写 Step1-7 组件，使用统一的 `DeptSignoffPanel` 会签组件。
...
model ProcessStepApproval {
  ...
  stepNumber  Int
  approverId  String
  approver    User            @relation("StepApprovalUser", fields: [approverId], references: [id])
  department  String
...
- [ ] **Step 2: 直接用 SQL 更新现有模板**（不破坏现有数据，用 upsert）
...
async function main() {
  ...
  const existing = await prisma.processTemplate.findFirst({ where: { isActive: true } });
  if (existing) {
    await prisma.processTemplate.update({ where: { id: existing.id }, data: { name: '产品研发流程（7步）', steps } });
    console.log('Updated template:', existing.id);
  } else {
    const t = await prisma.processTemplate.create({ data: { name: '产品研发流程（7步）', steps } });
...
export class ProcessStepApprovalService {
  ...
  async submitApproval(
    ...
    await this.prisma.processStepApproval.upsert({
      where: { instanceId_stepNumber_approverId: { instanceId, stepNumber, approverId } },
      update: { status: action === 'approve' ? 'APPROVED' : 'REJECTED', comment, signedAt: new Date(), department, role },
      create: { instanceId, stepNumber, approverId, department, role, status: action === 'approve' ? 'APPROVED' : 'REJECTED', comment, signedAt: new Date() },
    });
    ...
    const allApprovals = await this.prisma.processStepApproval.findMany({
      where: { instanceId, stepNumber },
    });
    const required: any[] = stepConfig.requiredApprovals ?? [];
    const allApproved = this.checkAllApproved(required, allApprovals);
    ...
    if (allApproved) {
      ...
      await this.advanceStep(instance, stepNumber, steps);
    }

    return { allApproved, rejected: false };
  }

  private checkAllApproved(required: any[], approvals: any[]): boolean {
  ...
  private async advanceStep(instance: any, stepNumber: number, steps: any[]) {
  ...
  async listPendingForUser(approverId: string) {
    return this.prisma.processStepApproval.findMany({
      ...
      include: { instance: { select: { id: true, productName: true, currentStep: true } } },
...
import {
  ...
  NotFoundException, UseGuards, Query,
} from '@nestjs/common';
...
将 `submitStep` 方法中 `if (!saveAsDraft) { ... }` 块替换为：

```typescript
    if (!saveAsDraft) {
      // Step3 无需审批人，提交即推进（trialConclusion==='通过'）
      const isStep3Pass = stepNumber === 3 && (data as any)?.trialConclusion === '通过';
      if (isStep3Pass) {
...
```typescript
  ...
  async submitApproval(
    ...
    const { action, comment = '', department = '', role = '' } = body;
    const result = await this.approvalService.submitApproval(
      id, parseInt(stepNumber), userId, action, comment, department, role,
    );
...
- [ ] **Step 5: 新增 Step1 副作用 —— 提交时创建 Product draft**

在 `submitStep` 方法末尾，`return` 语句之前插入：

```typescript
    // Step1 提交时创建 Product draft
    if (!saveAsDraft && (stepNumber === 1 || (!stepNumber && instance.currentStep === 1))) {
      const productName = (data as any)?.productName;
      if (productName && !instance.productId) {
        const existingCode = `RD-${Date.now()}`;
        ...
        await this.prisma.processInstance.update({
          where: { id },
          data: { productName, productId: product.id },
        });
      } else if (productName) {
        await this.prisma.processInstance.update({
          where: { id },
...
- [ ] **Step 6: 新增 Step5 副作用 —— 审批通过后同步营养成分到 Product**

在 `ProcessStepApprovalService.advanceStep` 方法中，`await this.prisma.processInstance.update(...)` 之后加：

```typescript
    // Step5 通过 → 同步营养成分到 Product
    if (stepNumber === 5 && instance.productId) {
      const stepData = await this.prisma.processStepData.findFirst({
        where: { instanceId: instance.id, stepNumber: 5 },
      });
      if (stepData?.data) {
        const d = stepData.data as any;
        await this.prisma.product.update({
          where: { id: instance.productId },
          data: {
            shelf_life_days: d.shelfLifeDays ? parseInt(d.shelfLifeDays) : undefined,
            nutrition_energy: d.nutritionEnergy ? parseFloat(d.nutritionEnergy) : undefined,
            nutrition_protein: d.nutritionProtein ? parseFloat(d.nutritionProtein) : undefined,
            nutrition_fat: d.nutritionFat ? parseFloat(d.nutritionFat) : undefined,
            nutrition_trans_fat: d.nutritionTransFat ? parseFloat(d.nutritionTransFat) : undefined,
            nutrition_carb: d.nutritionCarb ? parseFloat(d.nutritionCarb) : undefined,
            nutrition_sodium: d.nutritionSodium ? parseFloat(d.nutritionSodium) : undefined,
            product_type: d.productType,
            processing_method: d.processingMethod,
            standard_code: d.productStandard,
            storage_method: d.storageConditions,
            label_allergens: Array.isArray(d.allergens) ? d.allergens.join('、') : d.allergens,
          },
        });
      }
    }
...
- [ ] **Step 7: 新增 Step6 副作用 —— 审批通过后写入 Recipe/RecipeLine**

在同一个 `advanceStep` 方法中继续追加：

```typescript
    // Step6 通过 → 写入 Recipe + RecipeLine
    if (stepNumber === 6 && instance.productId) {
      const stepData = await this.prisma.processStepData.findFirst({
        where: { instanceId: instance.id, stepNumber: 6 },
      });
      if (stepData?.data) {
        const d = stepData.data as any;
        const recipeLines: any[] = d.recipeLines ?? [];
        if (recipeLines.length > 0) {
          ...
          await this.prisma.recipeLine.createMany({
            data: recipeLines
...
export const processApi = {
  ...
  // 多人会签
  submitApproval: (instanceId: string, stepNumber: number, payload: { action: 'approve' | 'reject'; comment?: string; department: string; role: string }) =>
...
<template>
  <div class="signoff-panel">
    ...
    <div v-if="canSign" class="sign-action">
      <el-input v-model="comment" placeholder="审批意见（可选）" style="width: 300px; margin-right: 12px" />
      <el-button type="success" @click="handleSign('approve')">同意</el-button>
      <el-button type="danger" @click="handleSign('reject')">驳回</el-button>
    </div>
    <div v-else-if="!disabled" class="sign-hint">
      <el-text type="info" size="small">当前用户无需在此步骤签署，或已完成签署</el-text>
    </div>
  </div>
</template>
...
const props = defineProps<{
  instanceId: string;
  stepNumber: number;
  myRole: string;       // 当前用户在此步骤的角色，如 'gm' | 'quality' | ''
  myDept: string;       // 当前用户的部门
...
const myRecord = computed(() =>
  approvalList.value.find(a => a.approverId === userStore.user?.id),
);

const canSign = computed(() =>
  !props.disabled &&
  props.myRole !== '' &&
  (!myRecord.value || myRecord.value.status === 'PENDING'),
);
...
- [ ] **Step 4: 修复上一步/下一步按钮的 disabled 判断**
...
- [ ] **Step 5: 在 handleApprove/handleReject 里改用新 API**
...
Expected: 出现 status='draft' 的新产品记录
