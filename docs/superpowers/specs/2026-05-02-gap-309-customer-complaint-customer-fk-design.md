...
- `docs/module-usage/08-traceability-complaint-recall.md`：GAP-309 标记为 P1、已验证，问题是 `CustomerComplaint.customer_name` 无 FK 关联 `Customer`/`ExternalParty` 主数据。
- `docs/module-usage/99-current-gap-register.md`：GAP-309 建议整改为 `CustomerComplaint` 新增客户 FK，`customer_name` 保留展示用。
