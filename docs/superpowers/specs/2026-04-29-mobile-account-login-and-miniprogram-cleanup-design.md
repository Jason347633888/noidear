...
8. 不实现异常上报、不合格/偏差/CAPA 快捷入口。
9. 不实现配料、投料、金检、包装、仓库收发料等深度业务工位专用流程。
...
### 6. H5 工位 Phase 1 功能范围

第一阶段只做“现场记录和待办入口”，不做异常闭环或深度生产工位。
...
- 不合格、偏差、CAPA 快捷入口。
- 离线缓存、离线提交、断网同步。
- 手机号、微信、短信、免密、工牌/NFC/PIN 登录。
- 设备原生能力深度集成。
- 配料、投料、金检、包装、仓库收发料等专用工位流程。
...
1. 登录成功后读取本地工位配置；没有配置时进入工位配置页。
2. 工位首页根据当前用户和工位配置请求待办、记录模板和最近记录。
3. 用户从待办或常用入口进入记录填写。
4. 表单字段需要设备、批次、物料、位置等上下文时，通过扫码或搜索选择主数据/批次数据，并保存对应 ID。
5. 用户提交记录后，后端写入 `Record` 或对应业务记录，并记录提交人、提交时间和来源上下文。
6. 首页和记录列表刷新最新状态。
...
处理：Phase 1 只做现场记录和待办入口。页面围绕“当前工位现在要做什么”组织，不复制 PC 后台的复杂管理能力。

### 风险：扫码输入只保存文本，后续无法追溯

处理：扫码可以作为输入手段，但提交前必须解析或选择到对应主数据/批次 ID。涉及产品、物料、设备、位置、生产批次、物料批次时，不允许只保存名称或扫码文本。
