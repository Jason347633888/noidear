...
3. 确认 `client/src/api/auth.ts` 当前不存在；`change-password` 需要新增账号 adapter 或放入明确账号 API 边界。
4. 跑一次旧范围扫描，记录 workflow、monitoring、internal-audit、management-review、SSO、asset-loan 的前端、后端、schema、seed、E2E 残留。

### 阶段 1：系统地图与删除范围

1. 修 `tools/generate-system-map.py`，加入递归 API adapter 扫描、direct request 扫描、controller 解析修正和 `deleted_scope` 分类。
