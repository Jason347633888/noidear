...
- `docs/module-usage/09-nonconformance-capa.md`：GAP-315 是 P1、已验证；当前 `NonConformance.disposition='rework'` 不自动创建 `ReworkRecord`，建议在事务中自动创建。
- `docs/module-usage/99-current-gap-register.md`：GAP-315 依赖 GAP-304 的 JWT `companyId` 多租户基础隔离；手动创建 `ReworkRecord` 路径不变。
