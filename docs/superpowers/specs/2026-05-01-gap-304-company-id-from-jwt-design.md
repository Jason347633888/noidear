...
本 spec 是主 agent 按 `docs/module-usage/00-index.md` 定义的流程生成：
...
`docs/module-usage/99-current-gap-register.md` 中 `GAP-304` 记录：
...
`docs/module-usage/09-nonconformance-capa.md` 还将同类问题扩展到：
...
`docs/module-usage/99-current-gap-register.md` 建议“从认证 JWT 中动态提取 company_id”，但当前认证代码不满足这个前提：
