...
GAP-311 的任务类型是 `needs_spec`。`ProductRecall` 在 `docs/MASTER_DATA_AND_TRACEABILITY_MODEL.md` 中被列为治理层对象，并明确处于“未独立建模”状态。当前 `docs/module-usage/08-traceability-complaint-recall.md` 也记录：召回仍通过 `RecordTemplate/Record` 动态表单处理，没有独立状态机、客户通知链、批次追溯范围和整改闭环。
