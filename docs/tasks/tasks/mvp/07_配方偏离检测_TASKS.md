...
**验收标准**:
- [ ] Prisma Schema 编写完成
- [ ] deviation_reports 表包含字段（id, record_id, field_name, expected_value, actual_value, deviation_amount, deviation_rate, reason, status, reported_at）
- [ ] 外键约束配置正确（record_id 引用 task_records(id)）
- [ ] 数据库迁移文件生成
