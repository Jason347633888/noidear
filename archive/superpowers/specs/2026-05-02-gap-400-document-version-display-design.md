...
- 历史 Multica GAP 模块文档：GAP-400 建议使用 `versionNo` 展示为 `V${versionNo}`，Decimal `version` 过渡期保留。
...
- 历史 Multica GAP 模块文档：受控文件事实源为 `Document`，允许展示字段包括 `versionNo`，并明确 `Document.version` 过渡期保留、后续用 `versionNo` 替代。
- 历史 Multica GAP 模块文档：GAP-400 为 P1，影响文控文件识别和合规审计，整改建议为使用 `versionNo` 和 `Vn` 展示。
- 历史 Multica GAP 模块文档：GAP-400 当前为 `needs_spec`，需走 `brainstorming -> grill-with-docs -> writing-plans`。
