...
- 历史 Multica GAP 模块文档：体系文件中心的事实源是 `Document`，文件发放记录是 `DocumentIssuance`；GAP-401 标记为 P1、已验证，问题是 `DocumentIssuance` 无 `documentId` 外键。
- 历史 Multica GAP 模块文档：GAP-401 记录为 schema/migration + 历史数据迁移影响，验收命令要求迁移后 `document_issuances.document_id IS NULL` 数量为 0。
