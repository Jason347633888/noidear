...
`EnvironmentRecord` 承载生产环境温湿度、压差等前提方案监控记录，属于品质管控与放行证据链的一部分。历史 Multica GAP 模块文档 已验证 GAP-302：`EnvironmentRecord.location` 仍是自由文本，无法稳定按位置维度查询，也无法证明记录位置来自受控主数据。
...
- 历史 Multica GAP 模块文档：当前代码中的区域主数据事实源是 `WorkshopArea`，`workshop_areas` 表是“区域主数据的唯一事实源”，旧代码写死或手填区域字符串不得扩大使用。
- 历史 Multica GAP 模块文档：GAP-302 建议“将 EnvironmentRecord.location 改为 FK 关联 Location 表，或新增 location_id 字段”。
- 历史 Multica GAP 模块文档：GAP-302 已验证，影响位置维度查询，建议 PR 为 `fix/environment-record-location-fk`。
...
- **grill-with-docs 校准结论：**
  ...
  - 不重复造主数据；当前代码没有 `Location` model，且 历史 Multica GAP 模块文档 已声明 `WorkshopArea` 是区域主数据唯一事实源。
