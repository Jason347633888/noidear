...
- 历史 Multica GAP 模块文档：GAP-316 标记为 P1、已验证；当前问题是 `CorrectiveAction.trigger_id` 可选且无来源校验，影响 CAPA 与触发来源双向查询。
- 历史 Multica GAP 模块文档：投诉触发 CAPA 时，`trigger_type='customer_complaint'` 且 `trigger_id` 应对应投诉 ID。
- 历史 Multica GAP 模块文档：内审发现项应进入 CAPA 闭环；当前 `CorrectiveAction.trigger_type` 已有 `internal_audit`，但无外键关联。
