...
`CCPRecord` 是生产批次放行证据链中的关键记录。历史 Multica GAP 模块文档 已明确：当 `CCPRecord.is_within_cl = false` 时，应进入 `NonConformance` 闭环，而不是只把偏差停留在 CCP 记录自身。
...
- 历史 Multica GAP 模块文档：GAP-305 已验证，建议在 `CCPRecord` 创建时，若 `is_within_cl=false`，自动创建 `NonConformance`。
- 历史 Multica GAP 模块文档：`NonConformance.source_type` 当前业务枚举为 `material_batch | production_batch | product`，上游批次来源应优先回到 `ProductionBatch` 或 `MaterialBatch`。
