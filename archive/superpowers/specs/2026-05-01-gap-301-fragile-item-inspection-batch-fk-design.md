...
`FragileItemInspection` 承载玻璃及硬塑完整性检查记录，是品质管控与放行模块中的异物控制证据。历史 Multica GAP 模块文档 将它与 `CCPRecord`、`EnvironmentRecord`、`ProcessMonitorRecord`、`MetalDetectionLog` 一起放在 `ProductionBatch -> ... -> 放行判断` 链路中。
...
- 历史 Multica GAP 模块文档：GAP-301 已验证，根因是 `FragileItemInspection.production_batch_id` nullable。
- 历史 Multica GAP 模块文档：GAP-301 涉及 schema/migration，不涉及历史数据迁移。
