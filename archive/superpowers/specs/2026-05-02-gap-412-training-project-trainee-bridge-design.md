...
- 历史 Multica GAP 模块文档：GAP-412 标记为 P2、已验证，建议为 TrainingProject 学员关系引入桥接表，替代 `trainees String[]`，并注明需要数据迁移。
- 历史 Multica GAP 模块文档：GAP-412 已回写为可执行计划，推荐 `brainstorming -> grill-with-docs -> writing-plans -> executing-plans`。
- 历史 Multica GAP 机器索引：GAP-412 当前 `triageStatus = ready_for_plan`，`specPath` 与 `planPath` 指向本 spec 和配套 plan。
