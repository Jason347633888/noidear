...
- 历史 Multica GAP 模块文档：`NonConformance.source_type` 当前支持 `'material_batch' | 'production_batch' | 'product'`；`source_id` 是对应来源 ID；GAP-313 建议应用层校验并添加索引。
- 历史 Multica GAP 模块文档：GAP-313 标记为 P1、已验证，建议 PR 为 `fix/nc-source-validation-and-index`。
