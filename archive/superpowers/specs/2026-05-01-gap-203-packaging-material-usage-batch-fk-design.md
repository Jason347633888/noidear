...
当前 历史 Multica GAP 模块文档、历史 Multica GAP 模块文档 和 Prisma schema 均已验证 GAP-203：
...
- 历史 Multica GAP 模块文档：已把包材用量定义为 `ProductionBatch -> PackagingMaterialUsage -> Material`，并标记 `production_batch_id` 无 FK 为 GAP-203。
- 历史 Multica GAP 模块文档：GAP-203 是 P1、已验证、涉及 schema/migration、不涉及历史数据迁移。
