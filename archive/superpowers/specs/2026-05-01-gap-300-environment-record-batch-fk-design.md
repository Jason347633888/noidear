...
`EnvironmentRecord` 承载生产环境温湿度、压差等前提方案监控记录。历史 Multica GAP 模块文档 已把它放在质量放行证据链中，与 `CCPRecord`、`ProcessMonitorRecord`、`MetalDetectionLog`、`FragileItemInspection` 一起支撑 `ProductionBatch` 是否可放行。
...
- 历史 Multica GAP 模块文档：GAP-300 已验证，根因是 `EnvironmentRecord.production_batch_id` nullable。
- 历史 Multica GAP 模块文档：GAP-300 涉及 schema/migration，不涉及历史数据迁移。
