...
- **grill-with-docs 校准结论：** 当前代码没有独立 `Location` model；历史 Multica GAP 模块文档 已定义 `WorkshopArea` 为区域主数据唯一事实源。因此本计划把 GAP-302 的“Location 主数据”落到 `WorkshopArea`，不得新建平行 `Location` 表。
