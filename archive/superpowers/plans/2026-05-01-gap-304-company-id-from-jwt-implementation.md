...
`rework-record` is referenced by 历史 Multica GAP 模块文档 as the same GAP-304 class, but it is not in the core `历史 Multica GAP 总表` evidence list. Include it only if doing so does not expand the PR into a broader quality-module cleanup.
