...
- `write_initial_plan`：只有 finalized spec/design 或 Design Final Digest，还没有 implementation plan。你必须调用 `writing-plans` skill 写第一版 implementation plan。
- `revise_plan`：已有 implementation plan。你负责回应质询，并在 Plan Final Digest 后修改 plan。
