...
issue 未关闭前，已启动的 teammate 必须保持可复用，不要因为单轮任务完成就 clean up。

Implementer 的任务必须是长生命周期任务：执行 plan、等待 review、处理返修、等待 Reviewer pass、继续等待 IssueLead closeout 信号。没有 `issuelead_closeout_started` 前，Implementer 不得把任务标记 complete，也不得主动退出当前 session。
...
- IssueLead 创建 implementation task 时，任务边界必须覆盖完整生命周期：执行 implementation plan、等待 Reviewer 审查、处理返修、等待 IssueLead closeout 信号。不得只创建“执行 plan”这种短任务。
- IssueLead 必须同时创建或保留一个同 issue 的 wait task，指向同一个 `Implementer`，内容为“等待 Reviewer 反馈、处理返修，并等待 IssueLead closeout 信号”；该 task 在 `issuelead_closeout_started` 前不得关闭。
...
- Reviewer 发现问题时，必须直接向同一个 Implementer 发送 repair 消息，并把摘要写入 `team-log.md`；IssueLead 只确认 wait task 存在。
- Implementer 对 finding 有疑问时，必须直接向同一个 Reviewer 质询；修完后必须直接通知同一个 Reviewer 复审当前 head，并同步 `implementation_ready_for_review` 给 IssueLead。
- 如果 Implementer 准备 idle/stop，但尚未收到 `issuelead_closeout_started`，视为流程错误：IssueLead 必须要求它继续等待，不新建替代 teammate，除非原 teammate 明确不可恢复。
...
返回后 `Implementer` 不退出、不释放上下文、不标记 task complete，状态记为：
...
Reviewer 返回 `review_passed_ready_for_closeout` 后，Implementer 仍需保持：

```text
implementer_status: waiting_for_issuelead_closeout
```

IssueLead 开始 closeout 前，必须先发送：

```text
issuelead_closeout_started
```

收到该信号后，Implementer 才允许停止等待。
